...
# The naming-contract machinery (B1) now lives in a shared service so BOTH this harness and the
# LangGraph code_generator agent bind every generation call to the same authoritative identifiers.
# Re-exported here under their original names so existing callers/tests (fc.build_naming_contract,
# fc._entities_from_sql, fc._prepend_contract, ...) keep working unchanged.
from app.services.naming_contract import (  # noqa: E402,F401
    _CONTRACT_MAX_CHARS,
    _CONTRACT_MAX_ENDPOINTS,
    _CONTRACT_MAX_ENTITIES,
    _endpoints_from_csv,
    _entities_from_mongo,
    _entities_from_sql,
    _pascal,
    _prepend_contract,
    _singularize,
...
# --------------------------------------------------------------------------- naming contract (B1)
#
# The naming-contract builder + injector now live in ``app/services/naming_contract.py`` (shared
# with the LangGraph code_generator agent) and are imported at the top of this module under their
# original names — ``build_naming_contract``, ``_entities_from_sql``, ``_prepend_contract``, etc. —
# so every caller here is unchanged. See that module for the full rationale.
